...
class Scene:
  ...
  def __init__(self,
    ...
    if os.path.exists(os.path.join(args.source_path, "sparse")):
      raise NotImplementedError("Colmap loading is not implemented yet!")
      scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path,
                                                    args.images, args.eval)
    elif os.path.exists(os.path.join(args.source_path,
    ...
    if shuffle:
      random.shuffle(
          scene_info.train_cameras)  # Multi-res consistent random shuffling
      random.shuffle(
          scene_info.test_cameras)  # Multi-res consistent random shuffling
    ...
    if self.loaded_iter:
      print(f"Loading Gaussians from iteration {self.loaded_iter}")
      self.gaussians.load_ply(
          os.path.join(self.model_path, "point_cloud",
                       "iteration_" + str(self.loaded_iter), "point_cloud.ply"))
    elif is_new_gaussian:
...
class BWScenes:
  ...
  def __init__(self, dataset, gaussians, is_new_gaussians=True):
    ...
    self.scene_white = Scene(dataset,
                             gaussians,
                             is_new_gaussian=is_new_gaussians)
    dataset.white_background = False
    self.scene_black = Scene(dataset,
                             gaussians,
                             is_new_gaussian=is_new_gaussians)
    self.background_black = torch.tensor([0, 0, 0],
                                         dtype=torch.float32,
                                         device="cuda")
    self.background_white = torch.tensor([1, 1, 1],
                                         dtype=torch.float32,
                                         device="cuda")
  ...
  def pop_black(self):
    self.load_stack()
    viewpoint_cam = self.viewpoint_stack_black.pop(
        randint(0,
                len(self.viewpoint_stack_black) - 1))
    return viewpoint_cam, self.background_black

  def pop_white(self):
    self.load_stack()
    viewpoint_cam = self.viewpoint_stack_white.pop(
        randint(0,
                len(self.viewpoint_stack_white) - 1))
