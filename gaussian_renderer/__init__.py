...
def render(viewpoint_camera,
  ...
  if opacity_filter is not None:
    retain_gs_mask = (opacity > opacity_filter).squeeze()
    means3D = means3D[retain_gs_mask]
    means2D = means2D[retain_gs_mask]
    opacity = opacity[retain_gs_mask]
    if scales is not None:
      scales = scales[retain_gs_mask]
    if rotations is not None:
      rotations = rotations[retain_gs_mask]
    if shs is not None:
      shs = shs[retain_gs_mask]
    if colors_precomp is not None:
      colors_precomp = colors_precomp[retain_gs_mask]
    if cov3D_precomp is not None:
      cov3D_precomp = cov3D_precomp[retain_gs_mask]
  ...
  (
      ...
      rendered_expected_depth) = rasterizer(means3D=means3D,
                                            means2D=means2D,
                                            shs=shs,
  ...
  if opacity_filter is not None:
    # We need to put back the radii into the original set of Gaussians, adding 0 for those
    # that were filtered out.
    full_radii = torch.zeros(screenspace_points.shape[0],
                             dtype=radii.dtype,
                             device=radii.device)
    full_radii[retain_gs_mask] = radii
    radii = full_radii
